...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message or not update.message.text:
        return
    match = re.search(r'\b([0-9a-fA-F]{64})\b', update.message.text.strip())
    if not match:
        return
...
async def recheck_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for tx_hash, data in not_found_list:
        try:
            await context.application.bot.send_message(
                ...
                text=(
                    f"⚠️ <b>Транзакция не найдена</b>\n\n"
                    f"Хеш: <code>{tx_hash}</code>\n\n"
                    f"Транзакция отсутствует в таблице. Свяжитесь с администратором."
                ),
                parse_mode="HTML"
            )
        except Exception as e:
            logger.error(f"Не удалось отправить личку: {e}")
        pending_checks.pop(tx_hash, None)

    await update.message.reply_text(
        f"✅ Готово!\nНайдено и обработано: {found_count}\nНе найдено (юзеры уведомлены): {len(not_found_list)}"
    )
